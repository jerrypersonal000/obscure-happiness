...
# Button layout: Clear and Replace buttons in the same row
...
# Debugging output to check final_value
st.write(f"Debug - final_value: {st.session_state.final_value}")
...
if A and B and C:
    try:
        ...
        if steps != -1:
            ...
            st.session_state.final_value = int(path[-1].split(" = ")[-1]) if path else A
        else:
